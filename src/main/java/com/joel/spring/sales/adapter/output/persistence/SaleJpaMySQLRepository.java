...
@Repository
public interface SaleJpaMySQLRepository extends JpaRepository<SaleEntity, String >{
    ...
    @Query("SELECT new com.joel.spring.sales.application.dto.SaleInfoDTO(s.id, s.totalPrice," +
            " new com.joel.spring.user.application.dto.UserPersonalInfoDTO(u.id, u.firstName, u.lastName, u.email)) FROM SaleEntity s JOIN UserEntity u WHERE s.id =:saleId")
